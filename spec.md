...
1. 产品经理创建产品，定义业务能力边界
2. 研发补充技术结构：创建子系统、应用、组件

### 研发开发提测流程

核心模型类比：每次组件版本发布 ≈ git commit，产品版本 ≈ 包含多个组件变更的聚合 commit。

1. **组件发布（持续进行）**
   - 研发构建并发布组件版本（ComponentVersion）
   - 每次发布自动生成一个新的产品版本（ProductVersion，状态为 draft）
   - 研发会频繁、多次发布不同组件，产品版本随之递增
   - 产品版本之间具有继承关系，形成版本链

2. **提交测试**
   - 研发选择当前最新的产品版本，提交进入待测试状态（draft → testing）
   - 该版本自动聚合「上一次验收通过版本 → 当前版本」之间的所有组件变更
   - 中间跳过的产品版本不需要逐一测试，测试只关注最终提测的版本

3. **测试验收**
   - 测试下载对应组件制品，逐组件验收
   - 验收通过：标记组件为通过，当所有组件均通过后，产品版本标记为已验收（testing → tested）
   - 验收失败：标记组件为失败，研发修复后发布新的组件版本，生成新的产品版本重新提测（不对同一版本重复提交）

4. **版本变更核心逻辑**
   - 研发提交版本记录（类似 git commit）
   - 测试拿到聚合后的 commit 进行验收
   - 最终标记该 commit 验收通过或失败

### 产品发布流程

1. 产品经理选择指定产品版本进行发布（版本可处于 tested / testing / draft 任意状态）
2. 发布后锁定版本内容，触发后端制品打包
